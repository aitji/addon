...
const KEY = "InvSee"
const instructions = `§cInvSee §7ติดตั้งแล้ว

§l§7#เพื่อดูไอเทมผู้เล่น §r§c(จำเป็น*)
§c-ใช้คำสั่ง      §7: !invsee §c[ชื่อผู้เล่น]
§c-แท็กแอดมิน   §7: Admin

§7ใช้คำสั่ง §c/tag @s add Admin §7เพื่อเข้าเป็นผู้ดูแล ของแอดออนนี้
§7ผู้ทำแอดออน §caitji §7(1.21.111)
...
world.afterEvents.playerSpawn.subscribe(({ player, initialSpawn }) => {
  ...
  const id = system.runInterval(() => {
    ...
    if (pos && !isNaN(pos.x)) {
      ...
      if (player.hasTag('op')) player.addTag('Admin') // migrate
